  ...
  pharmacy: {
    ...
    modules: [
      ...
      },
      {
        ...
        href: '/pharmacy/dispense',
        icon: Pill,
        description: 'Prescription dispensing & verification',
      ...
      },
      {
        ...
        href: '/pharmacy/stock',
        icon: Package,
        description: 'Inventory, reorder & expiry tracking',
      ...
      },
      {
        ...
        icon: AlertCircle,
        description: 'Controlled drug register & compliance',
    ...
    modules: [
      {
        ...
        href: '/patients',
        icon: Users,
        description: 'Patient profiles & dental history',
        color: 'border-blue-200 hover:border-blue-400',
        permissions: ['patients.read'],
        moduleCode: 'registration',
      },
      {
        ...
        href: '/dental/chart',
        icon: Smile,
        description: 'Interactive tooth charts & conditions',
      ...
      },
      {
        ...
        href: '/dental/procedures',
        icon: Scissors,
        description: 'Dental procedures & treatments',
        color: 'border-purple-200 hover:border-purple-400',
        permissions: ['encounters.read'],
        moduleCode: 'dental_charting',
      },
      {
        ...
        href: '/dental/treatment-plans',
        icon: ClipboardList,
        description: 'Multi-visit treatment planning',
      ...
      },
      {
        ...
        href: '/dental/imaging',
        icon: Scan,
        description: 'X-rays, OPG & imaging records',
      ...
      },
      {
        ...
        href: '/dental/lab-orders',
        icon: FlaskConical,
        description: 'Crown, bridge & prosthetics lab orders',
        color: 'border-orange-200 hover:border-orange-400',
        permissions: ['lab.read'],
        moduleCode: 'dental_charting',
      },
      {
        ...
        href: '/billing/invoices',
        icon: CreditCard,
        description: 'Invoices, insurance claims & payments',
        color: 'border-teal-200 hover:border-teal-400',
        permissions: ['billing.read'],
        moduleCode: 'billing',
      },
      {
        ...
        href: '/reports',
        icon: BarChart3,
        description: 'Practice analytics & performance',
        color: 'border-amber-200 hover:border-amber-400',
        permissions: ['reports.read'],
        moduleCode: 'reports',
      },
    ...
    modules: [
      {
        ...
        href: '/patients',
        icon: Users,
        description: 'Client profiles & visit history',
        color: 'border-blue-200 hover:border-blue-400',
        permissions: ['patients.read'],
        moduleCode: 'registration',
      },
      {
        ...
        href: '/optical/exams',
        icon: Eye,
        description: 'Comprehensive eye exams & refraction',
      ...
      },
      {
        ...
        href: '/optical/prescriptions',
        icon: ClipboardList,
        description: 'Spectacle & contact lens Rx',
        color: 'border-green-200 hover:border-green-400',
        permissions: ['encounters.read'],
        moduleCode: 'optical_exams',
      },
      {
        name: 'Spectacle Orders',
        href: '/optical/orders',
        icon: Glasses,
        description: 'Frame selection & lens cutting orders',
        color: 'border-pink-200 hover:border-pink-400',
        permissions: ['inventory.read'],
        moduleCode: 'optical_exams',
      },
      ...
      {
        name: 'Frame & Lens Inventory',
        href: '/optical/frames',
        icon: Package,
        description: 'Frame catalog & lens stock management',
        color: 'border-purple-200 hover:border-purple-400',
        permissions: ['inventory.read'],
        moduleCode: 'stores',
      },
      {
        ...
        href: '/billing/invoices',
        icon: CreditCard,
        description: 'Invoices, insurance & payments',
        color: 'border-teal-200 hover:border-teal-400',
        permissions: ['billing.read'],
        moduleCode: 'billing',
      },
      {
        ...
        href: '/reports',
        icon: BarChart3,
        description: 'Business analytics & performance',
        color: 'border-amber-200 hover:border-amber-400',
        permissions: ['reports.read'],
        moduleCode: 'reports',
      },
    ...
    modules: [
      {
        ...
        href: '/patients',
        icon: Users,
        description: 'Patient management & registration',
        color: 'border-blue-200 hover:border-blue-400',
        permissions: ['patients.read'],
        moduleCode: 'registration',
      },
      {
        ...
        href: '/encounters',
        icon: Stethoscope,
        description: 'Outpatient visits & consultations',
        color: 'border-green-200 hover:border-green-400',
        permissions: ['encounters.read'],
        moduleCode: 'doctors',
      },
      ...
      },
      {
        ...
        href: '/reports',
        icon: BarChart3,
        description: 'Insights, statistics & analytics',
        color: 'border-amber-200 hover:border-amber-400',
        permissions: ['reports.read'],
        moduleCode: 'reports',
      },
...
export default function DashboardPage() {
  ...
  const visibleQuickLinks = biz.quickLinks.filter(
    (link) => hasModuleAccess(link.moduleCode) && hasAnyPermission(link.permissions),
  );
  const visibleModules = biz.modules.filter(
    (mod) => hasModuleAccess(mod.moduleCode) && hasAnyPermission(mod.permissions),
  );
  ...
  const {
    ...
    queryFn: async () => {
      ...
      if (canReadEncounters) {
        promises.encounters = api
          .get('/encounters/stats/today')
          .catch(() => ({
            data: {
              total: 0,
              waiting: 0,
      ...
      if (canReadPharmacy) {
        promises.pharmacy = api
          .get('/pharmacy/queue/stats')
          .catch(() => ({ data: { pending: 0, dispensed: 0 } }));
      }
      if (canReadLab) {
        promises.lab = api
          .get('/lab/queue/stats')
          .catch(() => ({ data: { pending: 0, completed: 0 } }));
      }
      if (canReadIPD) {
        // Real bed counts. canReadIPD was already worked out here and then
        // never used, and the bed tile below was derived from admissions.
        promises.ipd = api.get('/ipd/stats').catch(() => ({ data: null }));
      }
      ...
      return {
        patients: {
          total:
            ...
            resolved.patients?.data?.meta?.total ||
            resolved.patients?.data?.total ||
            0,
          today: analytics?.patients?.newToday || 0,
        },
        encounters: resolved.encounters?.data || {
          total: 0,
          waiting: 0,
          inProgress: 0,
          completed: 0,
        ...
        // Beds come from the ward, not from admission counts. This used to
        // read total admissions as "total beds" and total-minus-active as
        // "beds available" — which is the count of *discharged* admissions.
        // A ward with four beds, two of them free, reported none available,
        // because both of its admissions were still open.
        beds: {
          total: resolved.ipd?.data?.totalBeds ?? 0,
          occupied: resolved.ipd?.data?.occupiedBeds ?? 0,
          available: resolved.ipd?.data?.availableBeds ?? 0,
        },
      } as DashboardStats;
    },
    staleTime: 30000,
              ...
              <>
                ...
                {canReadPharmacy && (
                  <>
                    <StatCardComponent
                      label="Rx Pending"
                      value={stats?.pharmacy.pending || 0}
                      icon={Pill}
                    ...
                    <StatCardComponent
                      ...
                      value={stats?.pharmacy.dispensed || 0}
                      icon={CheckCircle}
                ...
                {canReadBilling && (
                  <StatCardComponent
                    ...
                    value={stats?.billing.todayRevenue || 0}
                    icon={DollarSign}
                ...
                {canReadPharmacy && (
                  <StatCardComponent
                    ...
                    value={stats?.pharmacy.dispensed ? 0 : 0}
                    icon={Package}
                ...
                {canReadBilling && (
                  <StatCardComponent
                    ...
                    value={stats?.billing.pendingPayments || 0}
                    icon={CreditCard}
                    bgColor="bg-teal-100"
              ...
              <>
                ...
                {canReadEncounters && (
                  <>
                    <StatCardComponent
                      label="Waiting"
                      value={stats?.encounters.waiting || 0}
                      icon={Clock}
                      bgColor="bg-orange-100"
                      iconColor="text-orange-600"
                    />
                    <StatCardComponent
                      label="In Chair"
                      value={stats?.encounters.inProgress || 0}
                      icon={Smile}
                      bgColor="bg-pink-100"
                    ...
                    <StatCardComponent
                      ...
                      value={stats?.encounters.completed || 0}
                      icon={CheckCircle}
                      bgColor="bg-green-100"
                      iconColor="text-green-600"
                    />
        ...
        <div className="card border-none p-6 md:p-8">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-3">
            {visibleQuickLinks.map((link) => (
              ...
              >
                <div
                  className={`p-3 rounded-xl ${link.color} text-white shadow-sm group-hover:scale-110 transition-transform`}
                >
                  <link.icon className="w-5 h-5" />
                </div>
                <span className="text-xs font-semibold text-surface-700 text-center">
                  {link.name}
                </span>
              </Link>
            ))}
            ...
            {alerts && alerts.length > 0 ? (
              alerts.map((alert: any, index: number) => {
                ...
                return (
                  <div
                    key={index}
                    className={`flex items-start gap-3 p-3 ${bgColor} rounded-lg border`}
                  >
                    {alert.type === 'info' ? (
