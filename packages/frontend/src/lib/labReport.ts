...
function generateStandardPdf(data: LabReportData): jsPDF {
  ...
  y += 5;
  doc.text('*** End of Report ***', pw / 2, y + 4, { align: 'center' });
  ...
  for (let i = 1; i <= totalPages; i++) {
    ...
    if (footerParts.length > 0) {
      doc.text(footerParts.join('  |  '), pw / 2, ph - 12, { align: 'center' });
    }
...
function generateSimplifiedPdf(data: LabReportData): jsPDF {
  ...
  for (let i = 1; i <= totalPages; i++) {
    ...
    doc.setFont('helvetica', 'italic');
    doc.setTextColor(120);
    doc.text(`Generated by ${data.institution.name || 'Hospital'}`, 14, ph - 8);
