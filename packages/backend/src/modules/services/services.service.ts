...
export class ServicesService {
  ...
  async createCategory(dto: CreateServiceCategoryDto, tenantId?: string) {
    const exists = await this.categoryRepo.findOne({ where: { code: dto.code, ...(tenantId ? { tenantId } : {}) } });
    if (exists) throw new ConflictException('Category code already exists');
    return this.categoryRepo.save(this.categoryRepo.create({ ...dto, ...(tenantId ? { tenantId } : {}) }));
  }

  async findAllCategories(tenantId?: string) {
    return this.categoryRepo.find({ where: { isActive: true, ...(tenantId ? { tenantId } : {}) }, order: { sortOrder: 'ASC', name: 'ASC' } });
  }

  async updateCategory(id: string, dto: UpdateServiceCategoryDto, tenantId?: string) {
    const cat = await this.categoryRepo.findOne({ where: { id, ...(tenantId ? { tenantId } : {}) } });
  ...
  async createService(dto: CreateServiceDto, tenantId?: string) {
    const exists = await this.serviceRepo.findOne({ where: { code: dto.code, ...(tenantId ? { tenantId } : {}) } });
    if (exists) throw new ConflictException('Service code already exists');
    return this.serviceRepo.save(this.serviceRepo.create({ ...dto, ...(tenantId ? { tenantId } : {}) }));
  }

  async findAllServices(categoryId?: string, tier?: ServiceTier, includeInactive = false, tenantId?: string) {
  ...
  async findService(id: string, tenantId?: string) {
    const service = await this.serviceRepo.findOne({ where: { id, ...(tenantId ? { tenantId } : {}) }, relations: ['category'] });
  ...
  async createPrice(dto: CreateServicePriceDto, tenantId?: string) {
    return this.priceRepo.save(this.priceRepo.create({ ...dto, ...(tenantId ? { tenantId } : {}) }));
  }

  async getServicePrice(serviceId: string, tier: ServiceTier, facilityId?: string, tenantId?: string): Promise<number> {
  ...
  async createPackage(dto: CreateServicePackageDto, tenantId?: string) {
    const exists = await this.packageRepo.findOne({ where: { code: dto.code, ...(tenantId ? { tenantId } : {}) } });
    if (exists) throw new ConflictException('Package code already exists');
    return this.packageRepo.save(this.packageRepo.create({ ...dto, ...(tenantId ? { tenantId } : {}) }));
  }

  async findAllPackages(tenantId?: string) {
    return this.packageRepo.find({ where: { ...(tenantId ? { tenantId } : {}) }, order: { name: 'ASC' } });
  }

  async updatePackage(id: string, dto: Partial<CreateServicePackageDto> & { isActive?: boolean }, tenantId?: string) {
    const pkg = await this.packageRepo.findOne({ where: { id, ...(tenantId ? { tenantId } : {}) } });
