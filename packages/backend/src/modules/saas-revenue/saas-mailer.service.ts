...
export class SaasMailerService {
  ...
  constructor(
    ...
    if (host) {
      ...
      this.logger.warn(
        'SMTP not configured (SMTP_HOST missing) — emails will be logged to console only',
      );
    }
  }
  ...
  async getStoredTemplate(
    key: EmailTemplateKey,
    tenantId?: string,
  ): Promise<EmailTemplateStored | null> {
    try {
      const row = await this.settings.getByKey(`${TEMPLATE_KEY_PREFIX}${key}`, tenantId);
  ...
  async setTemplate(
    ...
    return this.settings.upsert(
      ...
      opts.tenantId,
      opts.tenantId
        ? `SaaS email template (tenant override): ${key}`
        : `SaaS email template: ${key}`,
    );
  }
  ...
  async resetTemplate(key: EmailTemplateKey, tenantId?: string) {
    try {
      await this.settings.delete(`${TEMPLATE_KEY_PREFIX}${key}`, tenantId);
    } catch {
      /* not set */
    }
  ...
  async listTemplates(tenantId?: string) {
    ...
    for (const meta of Object.values(EMAIL_TEMPLATES_META)) {
      let isCustom = false;
      try {
        await this.settings.getByKey(`${TEMPLATE_KEY_PREFIX}${meta.key}`);
        isCustom = true;
      } catch {
        /* default */
      }
      ...
      if (tenantId) {
        try {
          await this.settings.getByKey(`${TEMPLATE_KEY_PREFIX}${meta.key}`, tenantId);
      ...
      const historyCount =
        (tenantStored?.history?.length || 0) + (globalStored?.history?.length || 0);
  ...
  private async send(
    ...
    if (!this.transporter) {
      ...
      await this.persistLog({
        ...logEntry,
        status: 'sent',
        error: 'no SMTP configured (logged only)',
      });
      return;
    }
    try {
  ...
  private async persistLog(entry: Partial<SaasEmailLog>) {
    try {
      await this.emailLogRepo.save(this.emailLogRepo.create(entry));
    } catch (e: any) {
      this.logger.warn(`email log persist failed: ${e?.message}`);
    }
  }
  ...
  async listEmailLogs(
    ...
    if (opts.search)
      qb.andWhere('(l.subject ILIKE :q OR l.to ILIKE :q)', { q: `%${opts.search}%` });
  ...
  async sendDunning(
    ...
    return this.send(to, subject, html, {
      ...
      tenantId: inv.tenantId,
      invoiceId: inv.id,
      subscriptionId: _sub?.id ?? inv.subscriptionId,
    });
  }
    ...
    switch (key) {
      ...
      case 'renewal_reminder':
        return {
          planName: 'Professional',
          amount: 'UGX 15,000.00',
          renewalDate: new Date(Date.now() + 5 * 86400000).toLocaleDateString(),
          daysUntil: 5,
          currency: 'UGX',
        };
      case 'trial_ending':
        return {
          planName: 'Professional',
          trialEndDate: new Date(Date.now() + 3 * 86400000).toLocaleDateString(),
        };
    }
  }
  ...
  async previewTemplate(
    key: EmailTemplateKey,
    override?: Partial<EmailTemplate>,
    tenantId?: string,
  ): Promise<{ subject: string; html: string }> {
