...
export class PesapalService {
  ...
  private base = process.env.PESAPAL_BASE_URL
  ...
  isConfigured(): boolean { return !!(this.consumerKey && this.consumerSecret); }
  ...
  private async ensureIpnId(token: string, fallbackOriginUrl?: string): Promise<string> {
    ...
    const url = this.ipnUrlOverride
      || (fallbackOriginUrl ? `${fallbackOriginUrl.replace(/\/+$/, '')}/api/v1/saas-revenue/webhooks/pesapal` : null);
    if (!url) throw new BadRequestException('PESAPAL_IPN_URL not set and no origin available to register IPN');
  ...
  async initCheckout(args: PesapalInitArgs, originUrl?: string): Promise<{ link: string; provider: 'pesapal' | 'mock'; orderTrackingId?: string }> {
    ...
    if (!res.ok || !json.redirect_url) {
      ...
      throw new BadRequestException(json.error?.message || json.message || 'Failed to initiate Pesapal checkout');
    }
    return { link: json.redirect_url, provider: 'pesapal', orderTrackingId: json.order_tracking_id };
  }
