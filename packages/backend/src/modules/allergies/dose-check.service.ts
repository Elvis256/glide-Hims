...
const OTHER_UNITS = new Set(['iu', 'unit', 'units', 'u', 'puff', 'puffs', 'drop', 'drops', 'tab', 'tabs', 'cap', 'caps']);
...
export function normalizeDose(dose: ParsedDose, targetUnit?: string): { amount: number; unit: string } | null {
...
export function frequencyToPerDay(freq?: string): number | null {
  ...
  const map: Record<string, number> = {
    od: 1, qd: 1, daily: 1, 'once daily': 1, 'once a day': 1, hs: 1, nocte: 1,
    bd: 2, bid: 2, 'twice daily': 2, 'twice a day': 2,
    tds: 3, tid: 3, 'three times daily': 3, 'thrice daily': 3,
    qid: 4, qds: 4, 'four times daily': 4,
    '5x': 5, '5 times daily': 5,
    '6x': 6,
...
export class DoseCheckService {
  ...
  constructor(
    ...
    @InjectRepository(DrugClassification) private readonly classificationRepo: Repository<DrugClassification>,
  ...
  async runDoseChecks(input: DoseCheckInput): Promise<SafetyAlert[]> {
      ...
      try {
        ...
        if (age !== null && age < 18) {
          ...
          const enc = await this.encounterRepo.find({
            where: { patientId: input.patientId, ...(input.tenantId ? { tenantId: input.tenantId } : {}) },
            select: ['id'],
            take: 50,
    ...
    if (drugIds.length) {
      const classifications = await this.classificationRepo.find({
        where: drugIds.map((id) => ({ itemId: id, ...(input.tenantId ? { tenantId: input.tenantId } : {}) })) as any,
      });
