...
const COMMON_LAB_TESTS: LabTestReference[] = [
  ...
  },
  {
    ...
    specimen: 'Blood',
    units: 'g/dL',
    referenceRange: { low: 12.0, high: 17.0 },
  ...
  {
    code: '5196-1',
    name: 'Malaria Smear (Thick)',
    category: 'Infectious',
    specimen: 'Blood',
    units: 'Qual',
    referenceRange: { text: 'No parasites seen' },
  },
  {
    code: '75410-1',
  ...
  },
  {
    ...
    specimen: 'Urine',
    units: 'SG',
    referenceRange: { low: 1.005, high: 1.03 },
  },
  {
    code: '5804-0',
    name: 'Urine Protein',
  ...
  // Coagulation
  {
    ...
    specimen: 'Blood',
    units: 'seconds',
    referenceRange: { low: 11, high: 13.5 },
  },
  {
    code: '6301-6',
    name: 'INR',
...
export class LOINCService {
  ...
  searchLocal(query: string, limit = 20): LOINCCode[] {
    ...
    return COMMON_LAB_TESTS.filter(
      (t) =>
  ...
  getCommonLabTests(category?: string): LabTestReference[] {
    if (category) {
      return COMMON_LAB_TESTS.filter((t) => t.category.toLowerCase() === category.toLowerCase());
    }
  ...
  private mapFHIRToLOINC(parameters: any[], code: string): LOINCCode {
    const getParam = (name: string) => parameters.find((p) => p.name === name)?.valueString || '';
